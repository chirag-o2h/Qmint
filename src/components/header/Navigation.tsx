                  ...
                  return (
                    category?.subCategories?.length > 0 ?
                      <Fragment key={category.name}><HoverTooltip
                        ...
                        renderComponent={
                          <Link
                            to={frontPage ? `${formatCategoryUrl(category.searchEngineFriendlyPageName).replace(/\//g, '')}` : `/category/${formatCategoryUrl(category.searchEngineFriendlyPageName)?.replace(/\//g, '')}`}
