...
// Assets
import { ContainedCheckIcon, ContainedCrossIcon } from "../assets/icons/index";
...
function ResetPassword(params: any) {
  ...
  const { configDetails: configDetailsState, loadingForSignIn } =
  ...
  useLayoutEffect(() => {
    const verifyToken = async () => {
      ...
      const response: any = await dispatch(
        passwordRecoveryTokenVarified({
          url: ENDPOINTS.passwordRecoveryTokenVarified.replace(
            "{{token}}",
      ...
      if (isActionRejected(response.type)) {
        setLoginError(
          (
            (response.payload as AxiosError).response?.data as {
              message?: string;
            }
      ...
      if (!response?.payload?.data?.data) {
        setLoginError(
          response?.payload?.data?.message || "Something went wrong"
        );
      } else {
  ...
  const handleFormSubmit = async (data: any) => {
    ...
    const response: any = await dispatch(
      passwordRecoverySave({ url: ENDPOINTS.passwordRecoverySave, data: body })
    );
        ...
        <Stack id="BmkResetPassword">
          <Box className="LeftPart">
            <img
              ...
              src={configDetailsState?.Loginpage_Leftside_pic?.value}
              alt="left-image"
            />
          </Box>
